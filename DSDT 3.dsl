 ...
 *     Signature        "DSDT"
 *     Length           0x00012201 (74241)
 *     Revision         0x01 **** 32-bit table (V1), no 64-bit math support
 *     Checksum         0x06
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "TP-GN   "
 *     OEM Revision     0x00002400 (9216)
 *     Compiler ID      "INTL"
 *     Compiler Version 0x20120711 (538052369)
 */
DefinitionBlock ("", "DSDT", 1, "LENOVO", "TP-GN   ", 0x00002400)
{
    /*
    ...
    OperationRegion (GNVS, SystemMemory, 0x7CE63C18, 0x02B0)
    ...
    Scope (\_SB)
    {
        ...
        Device (PCI0)
        {
                ...
                If ((Arg0 == ToUUID ("33db4d5b-1ff7-401c-9657-7441c03dd766") /* PCI Host Bridge Device */))
                {
                    ...
                    If (!(CDW1 & 0x01))
                    {
                        If ((CTRL & 0x01))
                        {
                            ...
                            \_SB.PCI0.EXP5.HPCE = 0x00
                            \_SB.PCI0.EXP5.HPCS = 0x01
                            \_SB.PCI0.EXP5.PDC = 0x01
                            \NHPS = 0x01
                        }
                ...
                Device (VID)
                {
                    ...
                    Method (GPON, 1, NotSerialized)
                    {
                        If (ISOP ())
                        {
                            ...
                            ElseIf (((VSID != 0x221117AA) && (VSID != 0x221E17AA)))
                            {
                                \VHYB (0x04, 0x00)
                            }
                    ...
                    OperationRegion (ATRP, SystemMemory, \ATRB, 0x00018000)
                    ...
                    Method (_ROM, 2, Serialized)  // _ROM: Read-Only Memory
                    {
                        If ((Arg0 >= 0xC000))
                        {
                            Return (GETB ((Arg0 - 0xC000), Arg1, IDX1))
                        }
                        ...
                        Case (ToUUID ("e5c937d0-3553-4d7a-9117-ea4d19c3434d") /* Device Labeling Interface */){                            Switch (ToInteger (Arg2))
                            {
                                Case (0x00)
                                {
                                    If ((Arg1 == 0x02))
                                    {
                                        ...
                                        Return (OPTS) /* \_SB_.PCI0.EXP5.OPTS */
                                    }
                                    Else
                                    {
                                        Return (0x00)
            ...
            Device (SMBU)
            {
                ...
                Device (NFC)
                {
                    Name (_HID, EisaId ("BCM2F1B"))  // _HID: Hardware ID
                    Name (_CID, EisaId ("BCM2F1B"))  // _CID: Compatible ID
                    Name (_UID, 0x00)  // _UID: Unique ID
                    Method (_STA, 0, NotSerialized)  // _STA: Status
                    {
            ...
            Method (NVPS, 4, Serialized)
            {
                ...
                Switch (ToInteger (Arg2))
                {
                    ...
                    Case (0x2A)
                    {
                        Local0 = Buffer (0x24)
                            {
                                /* 0000 */  0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00,  // ........
                                /* 0008 */  0x3B, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // ;.......
                                /* 0010 */  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // ........
                                /* 0018 */  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  // ........
                                /* 0020 */  0x00, 0x00, 0x00, 0x00                           // ....
                            }
                        Local0 [0x00] = DerefOf (Arg3 [0x00])
                        Local0 [0x08] = \VHYB (0x07, 0x00)
    ...
    Method (\_PTS, 1, NotSerialized)  // _PTS: Prepare To Sleep
    {
        ...
        If (Local0)
        {
            ...
            If ((Arg0 >= 0x03))
            {
                ...
                \_SB.PCI0.LPC.TBTO = 0x00
            }

            If ((Arg0 != 0x05))
            {
                \_SB.PCI0.EXP3.PDSF = \_SB.PCI0.EXP3.PDS
                \_SB.PCI0.EXP5.PDSF = \_SB.PCI0.EXP5.PDS
            }
    ...
    Method (\_WAK, 1, NotSerialized)  // _WAK: Wake
    {
        ...
        If ((\_SB.PCI0.EXP5.PDS ^ \_SB.PCI0.EXP5.PDSF))
        {
            \_SB.PCI0.EXP5.PDSF = \_SB.PCI0.EXP5.PDS
            If ((TBTE == 0x00))
            {
                Notify (\_SB.PCI0.EXP5, 0x00) // Bus Check
            }
        }
